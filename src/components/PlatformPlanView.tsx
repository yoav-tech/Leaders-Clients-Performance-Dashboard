      ...
      {/* Content, one table per platform. A single list mixed Meta, TikTok and YouTube rows whose
        ...
        {exec.youtubeVideos.length > 0 && (
          <Panel title="תוכן · YouTube" note={`${exec.youtubeVideos.length} סרטונים · לפי פורמט`}>
            ...
            <div className="mt-2 text-[11px] text-[var(--muted)]">מקור: Google Ads API, קמפייני Leaders בלבד. השורות לפי כותרת הסרטון, וההוצאה היא זו שגוגל מחייבת עליו. שים לב ששמות המודעות בגוגל אדס לא תמיד תואמים לסרטון שרץ בתוכן. TrueView נספר אחרת בכל פורמט, ולכן העמודה שניתנת להשוואה בין הפורמטים ובין הפלטפורמות היא עלות לצפייה מלאה.</div>
          </Panel>
        )}
    </div>
  );
