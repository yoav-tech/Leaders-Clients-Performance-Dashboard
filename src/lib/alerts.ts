...
export async function performanceAlerts(days = 7): Promise<Alert[]> {
  ...
  for (const m of metrics) {
    ...
    if (!brand || brand.mediaPlan || brand.appInstall || brand.awarenessSources || brand.googleSnapshot || brand.perfSources) continue; // skip non-conversion brands
...
export async function adHealthAlerts(): Promise<Alert[]> {
  ...
  for (const brand of BRANDS) {
    if (brand.mediaPlan || brand.appInstall || brand.awarenessSources || brand.googleSnapshot || brand.perfSources) continue; // non-conversion brands: no alerts
